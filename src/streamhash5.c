...
static int STREAMHASH5_Init_NI(STREAMHASH5_CTX *ctx) {
...
/* XOR each state value with an input data block */
/* process the result with an AESENC round */
#define block_half(src) \
...
/* add a second AESENC round for full diffusion */
#define block_full(src) \
    block_half(src); \
    ctx->state[0] = _mm_aesenc_si128(ctx->state[0], consts[0]); \
    ctx->state[1] = _mm_aesenc_si128(ctx->state[1], consts[1]); \
    ctx->state[2] = _mm_aesenc_si128(ctx->state[2], consts[2]); \
    ctx->state[3] = _mm_aesenc_si128(ctx->state[3], consts[3])

static int STREAMHASH5_Update_NI(STREAMHASH5_CTX *ctx, const void *data, size_t len) {
...
static int STREAMHASH5_Final_NI(STREAMHASH5_CTX *ctx, void *md) {
    __m128i saved[4];
    ...
    block_half(saved[0]); block_half(saved[1]); block_half(saved[2]); block_half(saved[3]);
    block_half(saved[0]); block_half(saved[1]); block_half(saved[2]); block_half(saved[3]);
    block_half(saved[0]); block_half(saved[1]); block_half(saved[2]); block_half(saved[3]);
...
static int STREAMHASH5_Init_Portable(STREAMHASH5_CTX *ctx) {
    return 0; /* not implemented */
}

static int STREAMHASH5_Update_Portable(STREAMHASH5_CTX *ctx, const void *data, size_t len) {
    return 0; /* not implemented */
}

static int STREAMHASH5_Final_Portable(STREAMHASH5_CTX *ctx, void *md) {
    return 0; /* not implemented */
}
...
int STREAMHASH5_Init(STREAMHASH5_CTX *ctx) {
    if (implementation == IMPLEMENTATION_NONE)
        implementation = get_implementation();
#ifdef __AES__
    if (implementation == IMPLEMENTATION_NI)
        return STREAMHASH5_Init_NI(ctx);
#endif
    /* TODO: put other implementations here */
    return STREAMHASH5_Init_Portable(ctx);
}

int STREAMHASH5_Update(STREAMHASH5_CTX *ctx, const void *data, size_t len) {
    if (implementation == IMPLEMENTATION_NONE)
        implementation = get_implementation();
#ifdef __AES__
    if (implementation == IMPLEMENTATION_NI)
        return STREAMHASH5_Update_NI(ctx, data, len);
#endif
    /* TODO: put other implementations here */
    return STREAMHASH5_Update_Portable(ctx, data, len);
}

int STREAMHASH5_Final(STREAMHASH5_CTX *ctx, void *md) {
    if (implementation == IMPLEMENTATION_NONE)
        implementation = get_implementation();
#ifdef __AES__
    if (implementation == IMPLEMENTATION_NI)
        return STREAMHASH5_Final_NI(ctx, md);
#endif
    /* TODO: put other implementations here */
    return STREAMHASH5_Final_Portable(ctx, md);
}

int STREAMHASH5(const void *data, size_t len, void *md) {
    STREAMHASH5_CTX ctx;

    return STREAMHASH5_Init(&ctx) &&
        STREAMHASH5_Update(&ctx, data, len) &&
        STREAMHASH5_Final(&ctx, md);
}

/* end of streamhash5.c */
