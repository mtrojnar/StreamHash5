/* streamhash5sum.c - re-implemented basic features of the coreutils tool
 *
 * ISC License
 *
...
int hash(const char *name) {
    ...
    while (!feof(f)) {
        ...
        assert(STREAMHASH5_Update(&ctx, buff, inbytes) == 1);
        if (!inbytes && ferror(f)) {
            fprintf(stderr, "File read failed: %s\n", name);
            return 1;
        }
    }
    assert(STREAMHASH5_Final(&ctx, md) == 1);
    fclose(f);
