...
double benchmark(int len) {
    ...
    fprintf(stderr, "Doing streamhash5 for 3s on %d size blocks: ", len);
    fflush(stderr);
    msg = malloc(len);
    memset(msg, 0xc5, len);
    assert(STREAMHASH5(msg, len, out)==1); /* populate CPU caches */
    start = clock();
    end = start+3*CLOCKS_PER_SEC; /* expected value */
    for (n=0; clock() < end; n+=step)
        for(i=0; i<step; ++i)
            STREAMHASH5(msg, len, out);
    end = clock(); /* actual value */
    r = 0.001*n*len/(end-start)*CLOCKS_PER_SEC;
    free(msg);
    fprintf(stderr, "%d streamhash5's in %1.2fs\n",
...
int main() {
    ...
    printf("%-13s%12.2fk%12.2fk%12.2fk%12.2fk%12.2fk%12.2fk\n",
        "streamhash5", p16, p64, p256, p1024, p8192, p16384);
    return 0;
}
